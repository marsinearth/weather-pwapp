...
function convertGeoForOWMApi(geo) {
  const splittedGeo = geo.split(",");
  return `&lat=${splittedGeo[0]}&lon=${splittedGeo[1]}`;
}
...
function addLocation() {
  ...
  const convertedGeo = convertGeoForOWMApi(geo);

  getForecastFromNetwork(convertedGeo).then((forecast) => {
...
function renderForecast(card, data) {
  ...
  // css class name cannot starts with number so putting i there.
  card.querySelector(
    ".current .icon"
  ...
  futureTiles.forEach((tile, index) => {
    ...
    if (Array.isArray(forecast.weather)) {
      tile.querySelector(
        ".icon"
      ).className = `icon i${forecast.weather[0].icon}`;
    } else {
      tile.querySelector(".icon").className = `icon i${forecast.weather.icon}`;
    }
...
function updateData() {
  Object.keys(weatherApp.selectedLocations).forEach((key) => {
    ...
    const convertedGeo = convertGeoForOWMApi(location.geo);
    // CODELAB: Add code to call getForecastFromCache
    getForecastFromCache(convertedGeo).then((forecast) => {
      renderForecast(card, forecast);
    });
